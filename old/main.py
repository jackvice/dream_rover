...
def main(argv: list = None) -> None:
    ...
    Args:
        argv: Optional argument list to override default configurations.
    """
    ...
    # Set up arguments for running the script
    ...
    #    Initialize global timer if enabled
    def init():
        embodied.timer.global_timer.enabled = args.timer
    embodied.distr.Process.initializers.append(init)
    init()

    # Select the appropriate script to run based on the args.script
...
def make_env(config: embodied.Config, index: int, **overrides):
    ...
    Args:
        config: Configuration object for the environment settings.
        index: Index of the environment, useful in multi-environment setups.
        overrides: Additional overrides for specific environment settings.
    ...
    ctor = {
        ...
        'turtlbot': 'embodied.envs.turblebot:Turtlebot',
    }[suite]
...
def wrap_env(env, config: embodied.Config):
    ...
    Args:
        ...
        config: Configuration object for wrapping options.
