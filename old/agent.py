...
def sg(x): return treemap(jax.lax.stop_gradient, x)


cast = jaxutils.cast_to_compute


def sample(dist): return {
...
class Agent(nj.Module):
    ...
    def __init__(self, obs_space: dict, act_space: dict, config: embodied.Config):
        """
        Initialize the agent with observation and action spaces, and configuration.
        
        Args:
            obs_space: Dictionary containing observation space specifications.
            act_space: Dictionary containing action space specifications.
            config: Configuration object with various agent settings.
        """
        self.obs_space = {
    ...
    @property
    def aux_spaces(self) -> dict:
        """
        Return auxiliary spaces used during replay, including deterministic and stochastic states.
        """
        spaces = {}
        ...
        if self.config.replay_context:
            ...
            spaces['deter'] = embodied.Space(
                latdtype, self.config.dyn.rssm.deter)
            spaces['stoch'] = embodied.Space(
                np.int32, self.config.dyn.rssm.stoch)
        return spaces

    def init_policy(self, batch_size: int) -> tuple:
        """
        Initialize the agent's policy with default values.
        
        Args:
            batch_size: Size of the batch for the initial policy.

        Returns:
            A tuple containing the initial dynamics and previous actions.
        """
        prevact = {
    ...
    def init_train(self, batch_size: int) -> tuple:
        """
        Initialize the agent for training, similar to the policy initialization.
    ...
    def init_report(self, batch_size):
        """
        Initialize the agent for reporting, typically using the same initialization as training.

        Args:
            batch_size: Size of the batch for reporting.
    ...
    def policy(self, obs: dict, carry: tuple, mode: str = 'train') -> tuple:
        """
        The agent's policy function that determines the action to take based on observations.

        Args:
            obs: Dictionary of observations from the environment.
            carry: Tuple containing previous latent states and actions.
            mode: The mode in which the agent operates ('train', 'eval', etc.).

        Returns:
            A tuple containing the actions, auxiliary outputs, and updated latent states.
        """
    ...
    def train(self, data: dict, carry: tuple) -> tuple:
        """
        The training function where the agent updates its policy based on the given data.

        Args:
            data: Dictionary containing training data from the environment or replay buffer.
            carry: Tuple containing previous latent states and actions.

        Returns:
            A tuple containing outputs, updated carry (latent states and actions), and metrics.
        """
        self.config.jax.jit and embodied.print(
    ...
    def loss(self, data: dict, carry: tuple, update: bool = True) -> tuple:
        """
        Compute the loss for the agent based on the data and current state.

        Args:
            data: Dictionary containing training data.
            carry: Tuple containing previous latent states and actions.
            update: Boolean flag to determine if the model should update its parameters.

        Returns:
            A tuple containing the loss, auxiliary outputs, and metrics.
        """
        metrics = {}
        prevlat, prevact = carry
        ...
        newlat, outs = self.dyn.observe(
            prevlat, prevacts, embed, data['is_first'])
        ...
        # Annotate
        rew = jnp.concatenate(
            [startrew[:, None], self.rew(outs).mean()[:, 1:]], 1)
        con = jnp.concatenate(
            [startcon[:, None], self.con(outs).mean()[:, 1:]], 1)
        acts = sg(acts)
        inp = treemap({
            'none': lambda x: sg(x),
        ...
        logpi = sum([v.log_prob(sg(acts[k]))[:, :-1]
        ...
        if self.config.replay_critic_loss:
            ...
            rets = [boot[:, -1]]
            live = f32(~data['is_terminal'])[:, 1:] * \
                (1 - 1 / self.config.horizon)
            cont = f32(~data['is_last'])[:, 1:] * \
                self.config.return_lambda_replay
    ...
    def report(self, data, carry):
        ...
        losses = {k: -v.log_prob(data_img[k].astype(f32))
                  for k, v in img.items()}
        metrics.update({f'openl_{k}_loss': v.mean()
        ...
        for key in self.dec.imgkeys:
            ...
            pred = jnp.concatenate(
                [rec[key].mode()[:6], img[key].mode()[:6]], 1)
            error = (pred - true + 1) / 2
